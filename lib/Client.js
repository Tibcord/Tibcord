...
class Client extends EventEmitter {
    ...
    * @arg {String} activities[].type The type of the activity. 0 is playing, 1 is streaming (Twitch only), 2 is listening, 3 is watching, 5 is competing in
    ...
    */
    editStatus(status, activities) {
        ...
        if(activities === undefined && typeof status === "object") {
            activities = status;
            status = undefined;
            activities.type = ActivityTypes[activities.type]
        }
        if(status) {
        ...
        if(activities === null) {
            ...
            activities.map((presence) => ActivityTypes[presence.type]);
        }
