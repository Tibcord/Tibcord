...
/**
* Represents the main AriJS client
...
* @prop {Object} options AriJS options
...
* @prop {Collection<Shard>} shards Collection of shards AriJS is using
...
class Client extends EventEmitter {
    ...
    * @arg {Object} options AriJS client options
    ...
    * @arg {Boolean} [options.autoreconnect=true]  [DEPRECATED] Have AriJS autoreconnect when connection is lost. This option has been moved under `options.gateway`
    ...
    * @arg {Object} [options.disableEvents]  [DEPRECATED] If disableEvents[eventName] is true, the WS event will not be processed. This can cause significant performance increase on large bots. [A full list of the WS event names can be found on the docs reference page](/AriJS/docs/reference#ws-event-names). This option has been moved under `options.gateway`
    ...
    * @arg {Boolean} [options.gateway.autoreconnect=true] Have AriJS autoreconnect when connection is lost
    ...
    * @arg {Number | Array<String | Number>} [options.gateway.intents] A list of [intent names](/AriJS/docs/reference), pre-shifted intent numbers to add, or a raw bitmask value describing the intents to subscribe to. Some intents, like `guildPresences` and `guildMembers`, must be enabled on your application's page to be used. By default, all non-privileged intents are enabled.
    ...
    * @arg {Number | String} [options.gateway.maxConcurrency=1] The number of shards that can start simultaneously. If "auto" AriJS will use Discord's recommended shard concurrency.
    * @arg {Number | String} [options.gateway.maxShards=1] The total number of shards you want to run. If "auto" AriJS will use Discord's recommended shard count.
    ...
    * @arg {Number} [options.latencyThreshold=30000] [DEPRECATED] The average request latency at which AriJS will start emitting latency errors. This option has been moved under `options.rest`
    ...
    * @arg {Number | String} [options.maxShards=1] The total number of shards you want to run. If "auto" AriJS will use Discord's recommended shard count. This option has been moved under `options.gateway`
    ...
    * @arg {Number} [options.rest.latencyThreshold=30000] The average request latency at which AriJS will start emitting latency errors
    ...
    */
    getRESTChannel(channelID) {
        if(!this.options.restMode) {
            return Promise.reject(new Error("AriJS REST mode is not enabled"));
        }
