...
/**
* Represents the main apoloJS client
...
* @prop {Object} options apoloJS options
...
* @prop {Collection<Shard>} shards Collection of shards apoloJS is using
...
class Client extends EventEmitter {
    ...
    * @arg {Object} options apoloJS client options
    ...
    * @arg {Boolean} [options.autoreconnect=true]  [DEPRECATED] Have apoloJS autoreconnect when connection is lost. This option has been moved under `options.gateway`
    ...
    * @arg {Object} [options.disableEvents]  [DEPRECATED] If disableEvents[eventName] is true, the WS event will not be processed. This can cause significant performance increase on large bots. [A full list of the WS event names can be found on the docs reference page](/apoloJS/docs/reference#ws-event-names). This option has been moved under `options.gateway`
    ...
    * @arg {Boolean} [options.gateway.autoreconnect=true] Have apoloJS autoreconnect when connection is lost
    ...
    * @arg {Object} [options.gateway.disableEvents] If disableEvents[eventName] is true, the WS event will not be processed. This can cause significant performance increase on large bots. [A full list of the WS event names can be found on the docs reference page](//docs/reference#ws-event-names)
    ...
    * @arg {Number | Array<String | Number>} [options.gateway.intents] A list of [intent names](/apoloJS/docs/reference), pre-shifted intent numbers to add, or a raw bitmask value describing the intents to subscribe to. Some intents, like `guildPresences` and `guildMembers`, must be enabled on your application's page to be used. By default, all non-privileged intents are enabled.
    ...
    * @arg {Number | String} [options.gateway.maxConcurrency=1] The number of shards that can start simultaneously. If "auto" apoloJS will use Discord's recommended shard concurrency.
    * @arg {Number | String} [options.gateway.maxShards=1] The total number of shards you want to run. If "auto" apoloJS will use Discord's recommended shard count.
    ...
    * @arg {Number | Array<String | Number>} [options.intents] [DEPRECATED] A list of [intent names](//docs/reference), pre-shifted intent numbers to add, or a raw bitmask value describing the intents to subscribe to. Some intents, like `guildPresences` and `guildMembers`, must be enabled on your application's page to be used. By default, all non-privileged intents are enabled. This option has been moved under `options.gateway`
    ...
    * @arg {Number} [options.latencyThreshold=30000] [DEPRECATED] The average request latency at which apoloJS will start emitting latency errors. This option has been moved under `options.rest`
    ...
    * @arg {Number | String} [options.maxShards=1] The total number of shards you want to run. If "auto" apoloJS will use Discord's recommended shard count. This option has been moved under `options.gateway`
    ...
    * @arg {Number} [options.rest.latencyThreshold=30000] The average request latency at which apoloJS will start emitting latency errors
    ...
    */
    constructor(token, options) {
        ...
        if(!token || typeof(token) !== 'string') throw new Error('A valid token must be provided')
    ...
    * @returns {Promise<ApplicationCommand>}
    */
    createCommand(client_id = String, token = String, command) {
        if(!client_id || typeof(client_id) !== 'string' || !token || typeof(token) !== 'string') throw new Error('You need to specify the client ID and the client token to create commands.')
        if(command.name !== undefined && command.type === 1) {
            command.name = command.name.toLowerCase();
        }
        command.default_member_permissions = command.defaultMemberPermissions;
        ...
        command.description_localizations = command.descriptionLocalizations;
        command.name_localizations = command.nameLocalizations;
        return this.requestHandler.request("POST", Endpoints.COMMANDS(this.application.id || client_id), true, command, null, null, null, token).then((applicationCommand) => new ApplicationCommand(applicationCommand, this));
    }
    ...
    * @arg {String | Number} activities[].type The type of the activity. 0 is playing, 1 is streaming (Twitch only), 2 is listening, 3 is watching, 5 is competing in
    * @arg {String} [activities[].url] The URL of the activity
    * @arg {Number} [id] The id of the shard you want to change the status (optional)
    */
    editStatus(status, activities, id) {
        ...
        if(activities === null) {
            activities = [];
        } else if (!id && (typeof(activities) === 'number' || typeof(activities) === 'string')) {
            id = activities;
        } else if (activities && Array.isArray(activities)) {
            if (typeof activities.map((presence) => presence.type) === "string") {
                activities.map((presence) => presence.type = Constants.ActivityTypes[presence.type]);
            }
        } else if(activities && !Array.isArray(activities)) {
            if (typeof activities.type === "string") {
                activities.type = Constants.ActivityTypes[activities.type];
            }
        ...
        if(id) {
            ...
            this.shards.forEach((shard) => {
                if(shard.id === id) shard.editStatus(status, activities).catch(() => { throw new Error(`${id} is not a valid shard id.`)})
            });
        } else if(!id) {
            this.shards.forEach((shard) => {
    ...
    */
    getRESTChannel(channelID) {
        if(!this.options.restMode) {
            return Promise.reject(new Error("apoloJS REST mode is not enabled"));
        }
